...
const toggleActive = async () => {
  ...
  toggling.value = true
  try {
    await store.updateVacancy(item.value.id, { is_active: !item.value.is_active })
...
const deleteItem = async () => {
  const result = await Swal.fire({
    ...
    html: `<span style="color:#94a3b8">"${item.value.name}"</span> vakansiyasi o'chiriladi`,
    icon: 'warning',
  ...
  await store.removeVacancy(item.value.id)
  router.push({ name: 'vacancy-admin-list' })
}

onMounted(async () => {
  item.value = await store.fetchVacancy(route.params.id)
})
</script>
  ...
  <section class="page-section">
    <div class="cl-header">
      <div class="cl-header__left">
        <button class="cl-back-btn" @click="router.push({ name: 'vacancy-admin-list' })">
          <Icon icon="mdi:arrow-left" />
        </button>
        <div>
          <h2 class="cl-header__title">Vakansiya ma'lumotlari</h2>
    ...
    <template v-else>
      ...
      <BaseCard>
        <div class="show-grid">
          ...
          <div class="show-field">
            <span class="show-field__label">Maosh</span>
...
.show-icon { width:64px; height:64px; border-radius:18px; flex-shrink:0; background:linear-gradient(135deg,rgba(99,102,241,.25),rgba(59,130,246,.25)); border:1px solid rgba(99,102,241,.2); color:#818cf8; font-size:30px; display:flex; align-items:center; justify-content:center; }
