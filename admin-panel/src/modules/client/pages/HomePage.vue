...
onMounted(async () => {
  loadFromUrl()
  await Promise.all([fetchMasters(), fetchStats()])
})
</script>
...
.stats-card__arrow { width: 18px; height: 18px; color: rgba(255,255,255,0.7); flex-shrink: 0; }
...
.masters-section { display: flex; flex-direction: column; gap: 16px; }
.masters-section__toolbar { display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }
.masters-section__count { font-size: 14px; color: #374151; }
.masters-section__count strong { color: #111827; font-weight: 700; }
.toolbar-right { display: flex; align-items: center; gap: 8px; }

.filter-toggle-btn {
  display: none; align-items: center; gap: 6px; padding: 9px 14px;
...
.masters-section__sort {
  padding: 9px 32px 9px 12px; border: 1.5px solid #e5e7eb; border-radius: 10px;
  ...
  outline: none; cursor: pointer; appearance: none; font-family: inherit; transition: border-color 0.2s;
}
.masters-section__sort:focus { border-color: #2563eb; }

.masters-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; }
.masters-grid--loading { pointer-events: none; }
...
@media (max-width: 1100px) { .masters-grid { grid-template-columns: repeat(2, 1fr); } }
...
@media (max-width: 640px) {
  ...
  .masters-section__toolbar { flex-wrap: nowrap; }
}

@media (max-width: 520px) {
  .masters-grid { grid-template-columns: 1fr; }
  .stats-row__inner { grid-template-columns: 1fr 1fr; gap: 8px; }
  .stats-card { padding: 12px 14px; gap: 10px; }
  .stats-card__icon { width: 38px; height: 38px; border-radius: 10px; }
  .stats-card__number { font-size: 22px; }
  .stats-card__label { font-size: 11px; }
}
</style>
