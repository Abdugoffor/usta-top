...
const loadMore = async () => {
  ...
  try {
    const res = await getVacancies(buildParams(cursorToUse))
    const newItems = res.data?.data || []
    vacancies.value = [...vacancies.value, ...newItems]
    const meta = res.data?.meta || {}
    hasMore.value = meta.has_more || false
    nextCursor.value = meta.next_cursor || ''
  } catch {
...
onMounted(async () => {
  loadFromUrl()
  await Promise.all([fetchVacancies(), fetchMasterCount()])
      ...
      <div class="main-content__inner">
        <div class="sidebar-sticky">
          <FilterSidebar :model-value="filters" @update:model-value="Object.assign(filters, $event)"
            :show-categories="false" :mobile-open="mobileFilterOpen" @apply="applyFilters"
        ...
        <div class="vacancies-section">
          ...
          <template v-else>
            <div v-if="vacancies.length" class="vacancies-grid">
          ...
          <div v-if="!hasMore && vacancies.length > 0 && !loading" class="end-of-list">
...
.vacancies-section__count strong {
  color: #111827;
  font-weight: 700;
}
