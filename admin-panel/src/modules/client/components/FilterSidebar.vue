    ...
    <aside class="filter-sidebar" :class="{ 'filter-sidebar--mobile-open': mobileOpen }">
      ...
      <div v-if="showCategories" class="filter-sidebar__section">
        ...
        <div class="filter-sidebar__chips">
          <button v-for="(cat, idx) in categories" :key="cat.id" class="filter-chip"
        ...
        <div v-if="modelValue.category_ids?.length" class="filter-sidebar__selected-tags">
          <span v-for="catId in modelValue.category_ids" :key="catId" class="selected-tag">
      ...
      <div class="filter-sidebar__section">
        ...
        <div class="filter-sidebar__selects">
          <div class="filter-sidebar__select-wrap">
            ...
            <select :value="modelValue.region_id || ''" @change="selectRegion($event.target.value || null)"
              class="filter-sidebar__select">
          ...
          <div class="filter-sidebar__select-wrap" v-if="districts.length">
            ...
            <select :value="modelValue.district_id || ''" @change="selectDistrict($event.target.value || null)"
              class="filter-sidebar__select">
          ...
          <div class="filter-sidebar__select-wrap" v-if="mahallas.length">
            ...
            <select :value="modelValue.mahalla_id || ''" @change="selectMahalla($event.target.value || null)"
              class="filter-sidebar__select">
      ...
      <!-- Narx -->
      <div class="filter-sidebar__section">
        <button class="filter-sidebar__section-title filter-sidebar__section-title--btn"
        ...
        <div v-if="priceOpen" class="filter-sidebar__price">
          <div class="filter-sidebar__price-row">
            <input type="number" class="filter-sidebar__price-input" placeholder="Min" :value="modelValue.min_price"
              @input="update('min_price', $event.target.value)" />
            <span class="filter-sidebar__price-sep">—</span>
            <input type="number" class="filter-sidebar__price-input" placeholder="Max" :value="modelValue.max_price"
      ...
      <div class="filter-sidebar__section">
        ...
        <div class="filter-sidebar__status-btns">
          ...
          <button class="status-btn status-btn--green"
            :class="{ 'status-btn--active': modelValue.is_active_filter === 'active' }"
            @click="update('is_active_filter', 'active'); $emit('apply')">
            Faol
          </button>
          <button class="status-btn status-btn--gray"
            :class="{ 'status-btn--active': modelValue.is_active_filter === 'inactive' }"
...
.filter-sidebar__close-btn:hover {
  background: #f3f4f6;
}
...
.filter-sidebar__section-title svg {
  width: 16px;
  height: 16px;
}
...
.filter-sidebar__location-pill svg {
  width: 14px;
  height: 14px;
  flex-shrink: 0;
}

.filter-sidebar__location-pill span {
  flex: 1;
}

.filter-sidebar__location-pill button {
  background: none;
  border: none;
  cursor: pointer;
  color: #1d4ed8;
...
.filter-sidebar__location-pill--sm {
  background: #f0fdf4;
  color: #166534;
}

.filter-sidebar__location-pill--sm button {
  color: #166534;
}
...
.filter-sidebar__price-input::placeholder {
  color: #9ca3af;
}
...
.filter-sidebar__price-apply:hover {
  opacity: 0.9;
}

/* Holat 3-state tugmalari */
.filter-sidebar__status-btns {
  display: flex;
  gap: 6px;
}
