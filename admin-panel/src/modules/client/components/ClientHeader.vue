...
<template>
  <header class="site-header">
    <div class="site-header__inner">
      ...
      <div class="site-header__search">
        ...
        <input
          ...
          type="text"
          placeholder="Usta yoki ish qidirish..."
          :value="modelValue"
          @input="onInput"
          @keydown="onKeydown"
        />
      ...
      <nav class="site-header__nav">
        <RouterLink to="/" class="site-header__nav-link" exact-active-class="active">Ustalar</RouterLink>
        <RouterLink to="/vacancies" class="site-header__nav-link" active-class="active">Vakansiyalar</RouterLink>
      </nav>
      ...
      <div class="site-header__actions">
        <template v-if="auth.isLoggedIn">
          <div class="user-menu-wrap">
            ...
            <div v-if="userMenuOpen" class="user-dropdown" v-click-outside="() => userMenuOpen = false">
              <RouterLink to="/profile" class="user-dropdown__item" @click="userMenuOpen = false">
              ...
              <RouterLink to="/profile/resumes/create" class="user-dropdown__item" @click="userMenuOpen = false">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <RouterLink to="/profile/vacancies/create" class="user-dropdown__item" @click="userMenuOpen = false">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <template v-else>
          <RouterLink to="/login" class="site-header__btn site-header__btn--ghost">Kirish</RouterLink>
          <RouterLink to="/register" class="site-header__btn site-header__btn--primary">Ro'yxat</RouterLink>
        </template>
      </div>
    ...
    <div v-if="mobileMenuOpen" class="site-header__mobile-menu">
      ...
      <RouterLink to="/" class="site-header__mobile-link" @click="mobileMenuOpen = false">Ustalar</RouterLink>
      <RouterLink to="/vacancies" class="site-header__mobile-link" @click="mobileMenuOpen = false">Vakansiyalar</RouterLink>

      <template v-if="auth.isLoggedIn">
        <RouterLink to="/profile" class="site-header__mobile-link" @click="mobileMenuOpen = false">
          👤 {{ auth.user?.full_name }}
        </RouterLink>
        <RouterLink to="/profile/resumes/create" class="site-header__mobile-link" @click="mobileMenuOpen = false">
          📄 Resume qo'shish
        </RouterLink>
        <RouterLink to="/profile/vacancies/create" class="site-header__mobile-link" @click="mobileMenuOpen = false">
          💼 Vakansiya e'lon qilish
        </RouterLink>
        <button class="site-header__mobile-link site-header__mobile-link--danger" @click="doLogout" style="border:none;text-align:left;cursor:pointer;font-family:inherit;font-size:15px;">
          🚪 Chiqish
        </button>
      </template>
      <template v-else>
        <div class="site-header__mobile-actions">
          <RouterLink to="/login" class="site-header__btn site-header__btn--ghost" style="flex:1;text-align:center;text-decoration:none;" @click="mobileMenuOpen = false">Kirish</RouterLink>
          <RouterLink to="/register" class="site-header__btn site-header__btn--primary" style="flex:1;text-align:center;text-decoration:none;" @click="mobileMenuOpen = false">Ro'yxat</RouterLink>
        </div>
      </template>
    </div>
  </header>
...
.site-header__nav-link.active { background: #eff6ff; color: #2563eb; }
