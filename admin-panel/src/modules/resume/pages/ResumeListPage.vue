...
const reload = async () => {
  store.resetCursor()
  await store.fetchResumes(buildParams())
}
...
const goShow = (item) => router.push({ name: 'resume-admin-show', params: { id: item.slug || item.id } })

const deleteItem = async (item) => {
  const result = await Swal.fire({
    title: "O'chirishni xohlaysizmi?",
    html: `<span style="color:#94a3b8">"${item.name}"</span> resumesi o'chiriladi`,
    icon: 'warning',
  ...
  await store.removeResume(item.id)
  await reload()
  ...
  <section class="page-section">
    ...
    <div class="cl-header">
      <div class="cl-header__left">
        <div>
          <h2 class="cl-header__title">Resumelar</h2>
          <p class="cl-header__sub">Barcha foydalanuvchi resumelarini boshqaring</p>
        </div>
      </div>
    </div>
      ...
      <div class="cl-filters-row">
        <div class="cl-search">
          ...
          <input v-model="filters.search" type="text" class="cl-search__input"
            placeholder="Ism, kasb yoki aloqa bo'yicha qidirish..." autocomplete="off" />
    ...
    <BaseCard>
      ...
      <div v-else-if="!store.loading && store.total > 0" class="cl-pagination__info" style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border)">
        Jami: <strong>{{ store.total }}</strong> ta resume
      </div>
    </BaseCard>
