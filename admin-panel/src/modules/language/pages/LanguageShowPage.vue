...
onMounted(async () => {
  item.value = await store.fetchLanguage(route.params.id)
})
</script>
  ...
  <section class="page-section">
    <div class="cl-header">
      <div class="cl-header__left">
        <button class="cl-back-btn" @click="router.push({ name: 'language-list' })">
          <Icon icon="mdi:arrow-left" />
        </button>
        <div>
          <h2 class="cl-header__title">Til ma'lumotlari</h2>
          <p class="cl-header__sub">Batafsil ko'rish</p>
        </div>
      </div>
      <button class="cl-add-btn" @click="router.push({ name: 'language-edit', params: { id: route.params.id } })">
    ...
    <BaseCard v-else>
      <div class="show-grid">
        ...
        <div class="show-field">
          ...
          <span class="show-field__value">{{ item.description || '—' }}</span>
        </div>
        <div class="show-field">
