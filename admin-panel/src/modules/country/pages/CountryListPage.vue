  ...
  <section class="page-section">
    ...
    <BaseCard>
      <div class="cl-search">
        ...
        <input v-model="filters.name" type="text" class="cl-search__input" placeholder="Mamlakat nomini qidirish..."
          autocomplete="off" />
    ...
    <BaseCard>
      ...
      <!-- Tree rows -->
      <template v-else>
        <CountryTreeRow v-for="node in store.tree" :key="node.id" :node="node" :depth="0" @edit="goEdit"
          @delete="deleteItem" @add-child="n => goCreate(n.id)" />
      </template>
      ...
      <div class="cl-pagination" v-if="store.lastPage > 1">
        ...
        <div class="cl-pagination__btns">
          ...
          <button v-for="p in pageNumbers" :key="p" class="cl-pagination__btn"
            :class="{ 'cl-pagination__btn--active': p === store.currentPage }" @click="changePage(p)">{{ p }}</button>
          <span v-if="store.currentPage < store.lastPage - 3" class="cl-pagination__dots">…</span>
          <button v-if="store.currentPage < store.lastPage - 2" class="cl-pagination__btn"
            @click="changePage(store.lastPage)">{{ store.lastPage }}</button>
          <button class="cl-pagination__btn" :disabled="filters.page >= store.lastPage"
...
.cnt-header__col--status {
  width: 90px;
  text-align: center;
}

.cnt-header__col--actions {
  width: 110px;
  text-align: center;
}
...
.cnt-skeleton__cell--lg {
  width: 100%;
}

.cnt-skeleton__cell--sm {
  width: 70%;
}
